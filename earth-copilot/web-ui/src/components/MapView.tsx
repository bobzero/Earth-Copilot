...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  const [mobilityPinB, setMobilityPinB] = useState<{ lat: number; lng: number; marker: any } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (hasNewStacData) {
        ...
        console.log('[MSG] MapView: Plain text response (vision/chat) - preserving existing satellite data');
      ...
      if (isNavigateToAction) {
        ...
        if (selectedModule === 'extreme_weather' && navigateToData.latitude && navigateToData.longitude) {
          console.log('MapView: Auto-updating extreme weather pin to navigation center:', navigateToData.latitude, navigateToData.longitude);
          setPinState(prev => ({
            ...prev,
          ...
          if (lastChatResponse.data.stac_results.features && Array.isArray(lastChatResponse.data.stac_results.features)) {
            ...
            if (stacFeatures.length > 0) {
              ...
              if (queryBbox) {
                ...
                console.warn('MapView: No query bbox in translation_metadata, falling back to first feature bbox:', firstFeature.bbox);
              }
              ...
              if (allTileUrls && Array.isArray(allTileUrls) && allTileUrls.length > 1) {
                ...
                if (shouldLimitTiles) {
                  console.warn(`MapView: Found ${allTileUrls.length} tiles, limiting to ${MAX_TILES_TO_RENDER} for performance`);
                }
              ...
              if (useMosaicApproach) {
                ...
                if (shouldLimitMosaicTiles) {
                  console.warn(`MapView: Found ${stacFeatures.length} mosaic tiles, limiting to ${MAX_TILES_TO_RENDER} for performance`);
                }
                ...
                (async () => {
                  try {
                    ...
                    try {
                      ...
                      const signResponse = await authenticatedFetch(`${API_BASE_URL}/api/sign-mosaic-url`, {
                        method: 'POST',
                    ...
                    if (lastChatResponse?.data?.search_metadata?.spatial_extent) {
                      ...
                      console.log('MapView: No spatial_extent from backend, using original bbox:', bbox);
                    }
      ...
      if (lastChatResponse.data && lastChatResponse.data.stac_results) {
        ...
        if (features && features.length > 0) {
          ...
          if (shouldLimitTiles) {
            console.warn(`MapView: Found ${features.length} STAC items, limiting to ${MAX_TILES_TO_RENDER} for performance`);
            console.warn(`MapView: To see more tiles, zoom in or refine your query with date/cloud filters`);
          }
          ...
          if (!overallBbox && features.length > 0) {
            console.warn('MapView: No query bbox in translation_metadata, calculating from STAC features');
            let minLng = Infinity, minLat = Infinity, maxLng = -Infinity, maxLat = -Infinity;
            ...
            if (minLng !== Infinity && isFinite(minLng) && isFinite(minLat) && isFinite(maxLng) && isFinite(maxLat)) {
              overallBbox = [minLng, minLat, maxLng, maxLat];
              console.log('MapView: Calculated valid overall bbox from STAC features:', overallBbox);
            } else {
  ...
  const updateMapView = (bbox: number[] | null, minZoom?: number) => {
    if (map && bbox && bbox.length >= 4) {
      try {
        ...
        const clampedSouth = Math.max(-WEB_MERCATOR_LAT_LIMIT, Math.min(WEB_MERCATOR_LAT_LIMIT, south));
        const clampedNorth = Math.max(-WEB_MERCATOR_LAT_LIMIT, Math.min(WEB_MERCATOR_LAT_LIMIT, north));
        ...
        if (clampedSouth !== south || clampedNorth !== north) {
          console.warn(`MapView: Clamped latitudes to WebMercator limits: south ${south}->${clampedSouth}, north ${north}->${clampedNorth}`);
        }
        ...
        if (clampedSouth >= clampedNorth) {
          throw new Error(`Invalid bbox bounds after WebMercator clamping: south=${clampedSouth} >= north=${clampedNorth} (original: south=${south}, north=${north})`);
        }
    ...
    const processComparisonQuery = async () => {
      try {
        ...
        if (result.type === 'comparison' && result.before && result.after) {
          ...
          const hasBothTiles = result.before.tile_urls?.length > 0 && result.after.tile_urls?.length > 0;
          if (hasBothTiles) {
            ...
            (async () => {
              try {
                ...
                const stripPrefix = (s: string) => {
                  if (s.startsWith('data:image/png;base64,')) return s.replace('data:image/png;base64,', '');
                  if (s.startsWith('data:image/jpeg;base64,')) return s.replace('data:image/jpeg;base64,', '');
                  return s;
                };
                ...
                if (!beforeScreenshot && !afterScreenshot) {
                  console.warn('[SNAP] Comparison: Both screenshots failed — skipping Vision analysis');
                  return;
                }

                // 4) Send both screenshots to backend for GPT-5 Vision change detection
                console.log('[SNAP] Comparison: Sending before/after screenshots for Vision analysis...');
                if (onGeointAnalysis) {
                  onGeointAnalysis({
                    type: 'thinking',
                ...
                const visionResp = await authenticatedFetch(`${API_BASE_URL}/api/geoint/comparison`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    latitude: center_lat,
                ...
                if (visionResp.ok) {
                  ...
                  console.warn(`[SNAP] Comparison: Vision analysis request failed: ${visionResp.status}`);
                }
              } catch (visionErr) {
          ...
          if (onGeointAnalysis) {
            const analysisText = result.analysis || 'Comparison analysis completed but no imagery tiles were returned.';
            onGeointAnalysis({
              type: 'assistant',
              message: analysisText + (result.before || result.after ? '' : '\n\n*No before/after imagery tiles were found. Try a different collection, date range, or location.*')
            });
          }
        }
                  ...
                  if (isValidBounds(unionBbox)) {
                    ...
                    const allTileUrls = stacFeatures.map((feature: { id: string; bbox?: number[]; collection?: string; assets?: { tilejson?: { href?: string } } }) => {
                      ...
                      if (backendTilejsonUrl) {
                        ...
                        console.warn(`MapView: No tilejson in assets for ${feature.id}, using fallback`);
                      }
  ...
  const handleModuleSelect = (module: string) => {
    ...
    if (module === 'vision') {
      ...
      message = '**Building Damage Assessment Selected**\n\nDrop a pin on the building you want to analyze.';
    } else if (module === 'extreme_weather') {
      message = '**Extreme Weather Analysis Selected**\n\nDrop a pin on the location you want to analyze.';
    } else if (module === 'timeseries') {
      message = '**Time Series Animation Selected**\n\nDrop a pin on the location you want to analyze.';
    }
  ...
  const handleMapClickForPin = async (lat: number, lng: number) => {
    if (!pinMode || !map || !selectedModule) {
      console.log('[PIN] MapView: Pin drop cancelled - pinMode:', pinMode, 'selectedModule:', selectedModule);
      return;
    }
    ...
    if (selectedModule === 'comparison') {
      ...
      try {
        ...
        if (captured) {
          ...
          else if (clean.startsWith('data:image/jpeg;base64,')) clean = clean.replace('data:image/jpeg;base64,', '');
          if (clean.length > 1000) {
            setVisionScreenshot(clean);
            console.log(`[SNAP] MapView: Comparison pin screenshot captured (${Math.round(clean.length / 1024)}KB)`);
          }
        }
      } catch (snapErr) {
    ...
    if (selectedModule === 'mobility') {
      console.log(`[PIN] MapView: Mobility two-pin workflow at (${lat.toFixed(4)}, ${lng.toFixed(4)})`);
      ...
      try {
        ...
        if (screenshot) {
          ...
          if (base64.length > 1000) {
            ...
            console.log(`[SNAP] MapView: Mobility screenshot captured (${Math.round(base64.length/1024)}KB)`);
          }
        }
      } catch (e) {
    ...
    console.log(`[PIN] MapView: Placing pin at (${lat.toFixed(4)}, ${lng.toFixed(4)}) for module: ${selectedModule}`);
      ...
      if (onGeointAnalysis) {
        ...
        if (selectedModule === 'building_damage') {
          ...
          if (currentZoomLevel < 16) {
            console.log(`MapView: Building damage zoom check failed (zoom=${currentZoomLevel}, need >= 16)`);
            onGeointAnalysis({
              type: 'error',
          ...
          if (capturedScreenshot) {
            ...
            console.log(`MapView: Building damage screenshot captured (${Math.round(cleanScreenshot.length/1024)}KB)`);
          ...
          if (capturedScreenshot) {
            ...
            console.log(`MapView: Extreme weather screenshot captured (${Math.round(cleanScreenshot.length/1024)}KB)`);
            setVisionScreenshot(cleanScreenshot);
          } else {
            ...
            if (capturedScreenshot) {
              screenshot = capturedScreenshot.startsWith('data:image/png;base64,')
                ...
                : capturedScreenshot.startsWith('data:image/jpeg;base64,')
                  ...
                  : capturedScreenshot;
              console.log(`MapView: Screenshot captured for ${selectedModule} (${Math.round(screenshot.length/1024)}KB)`);
            }
          }
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('MapView: Updated map context for Chat Vision:', {
        ...
        screenshot_size: visionScreenshot ? `${Math.round(visionScreenshot.length/1024)}KB` : 'none',
  ...
  }, [satelliteData, map, mapLoaded, mapProvider, onMapContextChange, lastCollection, visionMode, visionPin, visionScreenshot, mapPositionVersion, selectedModule]);
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    cursor: 'pointer',
                    border: selectedModule === 'extreme_weather' ? '2px solid #f97316' : '1px solid rgba(0, 0, 0, 0.1)',
                    background: selectedModule === 'extreme_weather' ? 'rgba(249, 115, 22, 0.1)' : 'white',
                    transition: 'all 0.2s ease'
                  }}
                  onMouseEnter={(e) => {
          ...
          {comparisonMode && comparisonState.beforeImagery && comparisonState.afterImagery && (
            <div style={{
              ...
              <div
                ...
                onMouseEnter={(e) => {
                  if (!comparisonState.showingBefore) e.currentTarget.style.background = 'rgba(0, 0, 0, 0.05)';
                }}
                onMouseLeave={(e) => {
                  if (!comparisonState.showingBefore) e.currentTarget.style.background = 'rgba(255, 255, 255, 0.9)';
                }}
              >
                Before
              </div>
