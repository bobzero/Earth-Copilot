    ...
    const handleQueryEvent = (event: CustomEvent<{ 
      ...
      if (query && query.trim()) {
        ...
        requestAnimationFrame(() => {
          ...
          // ============================================================
          // VALIDATION: Check requirements for Vision module queries
          // ============================================================
          const validationErrors: string[] = [];
          
          // Check if Vision module is ON
          // Use multiple sources: selectedModule prop OR mapContext.vision_mode flag
          // This handles cases where state propagation is delayed
          const isVisionActive = currentSelectedModule === 'vision' || currentMapContext?.vision_mode === true;
          
          if (requiresVision && !isVisionActive) {
            console.warn(' Chat: Vision validation failed - selectedModule is:', currentSelectedModule, 'vision_mode:', currentMapContext?.vision_mode);
            validationErrors.push('**Vision module is not active.** Please click the Vision button in the module panel first.');
          }
          
          // Check if a pin has been dropped (vision_pin exists in mapContext)
          if (requiresPin && !currentMapContext?.vision_pin) {
            validationErrors.push('**No pin dropped.** After enabling Vision mode, click on the map to drop a pin at the location you want to analyze.');
          }
          
          // Check if STAC data is loaded (has tiles or stac_items)
          if (requiresStacData) {
            const hasStacData = ((currentMapContext?.tile_urls?.length ?? 0) > 0) || ((currentMapContext?.stac_items?.length ?? 0) > 0);
            if (!hasStacData) {
          ...
          if (validationErrors.length > 0) {
            console.warn(' Chat: Vision query validation failed:', validationErrors);
            const errorMessage: ChatMessage = {
              role: 'assistant',
              content: `Cannot run Vision analysis yet. Please navigate to the geointelligence modules, click on the vision module and drop a pin first.`,
              timestamp: new Date()
            };
  ...
  const chatMutation = useMutation({
    mutationFn: async (message: string) => {
      try {
        ...
        const hasVisionContext = selectedModule === 'vision' && currentMapCtx?.vision_mode && currentMapCtx?.vision_pin;
        
        if (hasVisionSession || hasLocalVisionSession || hasVisionContext) {
          // Use session coordinates or mapContext coordinates
          const lat = visionSession?.lat ?? currentMapCtx?.vision_pin?.lat ?? 0;
          const lng = visionSession?.lng ?? currentMapCtx?.vision_pin?.lng ?? 0;
          ...
          console.log(' Chat: Routing to vision agent', {
            hasVisionSession,
            ...
            lat,
            lng,
            hasScreenshot: !!currentMapCtx?.imagery_base64,
            hasTileUrls: !!(currentMapCtx?.tile_urls?.length),
            collection: currentMapCtx?.current_collection,
            analysisType: pendingAnalysisType
          });
          
          // Create enhanced map context with analysis type hint
          const enhancedMapContext = {
            ...currentMapCtx,
            analysis_type: pendingAnalysisType  // Pass hint to backend
          };
          ...
          const result = await sendVisionChatMessage(
            ...
            enhancedMapContext  // Pass enhanced map context with analysis_type hint
          );
          
          console.log(' Chat: Vision agent response:', result);
          console.log(' Chat: Tool calls:', result.tool_calls);
          
          // Clear the analysis type hint after use
          setPendingAnalysisType(null);
          
          //  Store session ID for follow-up questions
          if (result.session_id && !localVisionSessionId) {
            console.log(' Chat: Storing vision session ID for follow-ups:', result.session_id);
        ...
        const result = await apiService.sendChatMessage(message, selectedDataset?.id, conversationId, messages, currentPin || undefined, geointMode, freshMapContext, selectedModel);
