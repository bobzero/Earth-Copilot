...
@description('Set to true to restore a soft-deleted Cognitive Services account with the same name. Required when redeploying after a resource group deletion if the account is still within the 48-hour soft-delete retention period. Use `az cognitiveservices account list-deleted` to check.')
...
resource aiFoundry 'Microsoft.CognitiveServices/accounts@2025-04-01-preview' = {
  ...
  properties: {
    ...
    restore: restoreSoftDeletedAccount  // Set to true via parameter when redeploying over a soft-deleted account
