...
@description('Existing Azure AI Foundry project endpoint used when deployAIFoundry is false')
param azureAiProjectEndpoint string = ''

@description('Azure OpenAI deployment name available on the selected AI service')
param azureOpenAiDeploymentName string = 'gpt-4o'

@description('Fast Azure OpenAI deployment name available on the selected AI service')
param azureOpenAiFastDeployment string = 'gpt-4o-mini'

@description('Use the Container App managed identity for the selected AI service instead of an API key')
...
@description('Deploy Azure AI Foundry resources. Keep false when using an existing shared AI service.')
...
module web './app/web.bicep' = if (!empty(containerImage)) {
  ...
  params: {
    ...
    azureOpenAiFastDeployment: azureOpenAiFastDeployment
    useManagedIdentity: deployAIFoundry ? true : useManagedIdentity
    ...
    azureAiProjectEndpoint: deployAIFoundry ? (aiFoundry.?outputs.?agentProjectEndpoint ?? '') : azureAiProjectEndpoint
