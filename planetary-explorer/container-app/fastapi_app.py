...
# Profile lookup is used inside build_stac_query() to detect static
# collections (no datetime field). Imported defensively to avoid hard
# coupling -- if the module fails to import the lookup degrades to an
# empty dict and the previous always-add-sortby behavior is preserved.
try:
    from collection_profiles import COLLECTION_PROFILES as _COLLECTION_PROFILES
    ...
    *,
    ...
    if n == 0:
        ...
        return (
            ...
            f"{(' ' + date_phrase) if date_phrase else ''}. "
            "Try a different collection, a wider area, or a different time range."
        )
...
def build_stac_query(stac_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if stac_params.get('sortby'):
        ...
        # EXCEPTION: Static collections (e.g. Copernicus/ALOS DEM, JRC GSW) have
        # no datetime field on items. Adding `sortby: datetime` against them
        # makes MPC STAC return HTTP 400 / zero features, surfacing as the
...
async def unified_query_processor(request: Request):
    ...
    try:
        ...
        # ----------------------------------------------------------------
        # QUICKSTART SHORT-CIRCUIT — Get Started buttons / vision-module
        # STAC prompts route as a PURE Planetary Computer STAC search ->
        # tiles, NOT as agent turns.
        #
        # These are exact-match, pre-computed demo queries (location +
        # collection + optional date). They are fully specified, so there
        # is nothing to route, plan, analyze, or clarify. Running the v2
        # pipeline on them is both wasteful and harmful: the ActionRouter
        # can mislabel phrasings like "...daily activity..." or "...changes
        # ..." as LOAD_AND_ANALYZE / ANALYZE, which drops into the
        # AnalystAgent and asks the user a clarifying question for a query
        # that needs none (repro: "Show fire modis thermal anomalies daily
        # activity for Australia from June 2025" -> "Terra, Aqua, or both?
        # what date window?" instead of just rendering the tiles).
        #
        # We detect the exact quickstart cache hit BEFORE the pipeline and
        # skip it entirely by leaving v2_result empty, so every
        # `v2_result.get("action") == ...` handler below is a no-op and
        # control falls through to the deterministic quickstart fast path
        # (which performs the STAC search and returns the tiles). Pro mode
        # is excluded because the cached public collections (naip,
        # sentinel-2, modis-*, hls2-*, ...) do not exist in MPC Pro.
        # ----------------------------------------------------------------
        _qs_pipeline_mode = (
        ...
        if _is_quickstart_turn:
            logger.info(
                "[FAST] quickstart exact-match '%s' -> skipping v2 pipeline "
                "(no router / no LoadAgent / no AnalystAgent / no clarify); "
                "routing straight to STAC fast path",
                natural_query,
            )
            v2_result = {}
        else:
