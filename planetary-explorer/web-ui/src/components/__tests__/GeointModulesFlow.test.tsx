...
const { triggerGeointAnalysisMock } = vi.hoisted(() => ({
  triggerGeointAnalysisMock: vi.fn()
}));

vi.mock('../../services/api', async (importOriginal) => ({
  ...await importOriginal<typeof import('../../services/api')>(),
  triggerGeointAnalysis: triggerGeointAnalysisMock
}));
...
describe('GEOINT Modules End-to-End Flow', () => {
  beforeEach(() => {
    ...
    // Clear all mocks
    vi.clearAllMocks();
    triggerGeointAnalysisMock.mockResolvedValue({
      result: {
  ...
  describe('Error Handling', () => {
    it('should handle GEOINT analysis API failure gracefully', async () => {
      const onGeointAnalysis = vi.fn();
      triggerGeointAnalysisMock.mockRejectedValue(new Error('API connection failed'));
