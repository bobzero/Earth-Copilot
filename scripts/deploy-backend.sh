...
CONFIGURED_AZURE_OPENAI_ENDPOINT="${AZURE_OPENAI_ENDPOINT:-}"
CONFIGURED_AZURE_AI_PROJECT_ENDPOINT="${AZURE_AI_PROJECT_ENDPOINT:-}"
CONFIGURED_AZURE_OPENAI_KEY="${AZURE_OPENAI_API_KEY:-}"
CONFIGURED_AZURE_OPENAI_DEPLOYMENT_NAME="${AZURE_OPENAI_DEPLOYMENT_NAME:-}"
CONFIGURED_AZURE_OPENAI_FAST_DEPLOYMENT="${AZURE_OPENAI_FAST_DEPLOYMENT:-}"
...
resolve_openai() {
  if [ -n "$CONFIGURED_AZURE_OPENAI_ENDPOINT" ]; then
    AZURE_OPENAI_NAME=""
    AZURE_OPENAI_ENDPOINT="$CONFIGURED_AZURE_OPENAI_ENDPOINT"
    AZURE_OPENAI_KEY="$CONFIGURED_AZURE_OPENAI_KEY"
    AZURE_OPENAI_DEPLOYMENT_NAME="${CONFIGURED_AZURE_OPENAI_DEPLOYMENT_NAME:-gpt-4o}"
    AZURE_OPENAI_FAST_DEPLOYMENT="${CONFIGURED_AZURE_OPENAI_FAST_DEPLOYMENT:-gpt-4o-mini}"
    AZURE_OPENAI_AVAILABLE_MODELS="$AZURE_OPENAI_DEPLOYMENT_NAME,$AZURE_OPENAI_FAST_DEPLOYMENT"
    echo "  Azure OpenAI: using configured shared endpoint"
...
# Bind the OpenAI key as a Container App secret instead of a plaintext env var.
# Sets OPENAI_KEY_ENV_PAIR for --set-env-vars / --env-vars.
configure_openai_secret() {
  ...
  if [ "$USE_MANAGED_IDENTITY" = "true" ]; then
    ...
    az containerapp secret set \
      ...
      --output none
    OPENAI_KEY_ENV_PAIR="AZURE_OPENAI_API_KEY=secretref:azure-openai-api-key"
  else
    OPENAI_KEY_ENV_PAIR="AZURE_OPENAI_API_KEY="
  fi
}
...
update_container_app() {
  ...
  configure_openai_secret "$CA_NAME"
  ...
  az containerapp update \
    ...
    --set-env-vars \
      ...
      "AZURE_AI_PROJECT_ENDPOINT=${CONFIGURED_AZURE_AI_PROJECT_ENDPOINT}" \
      "${OPENAI_KEY_ENV_PAIR}" \
...
create_container_app() {
  ...
  OPENAI_CREATE_SECRET_ARGS=()
  if [ "$USE_MANAGED_IDENTITY" != "true" ] && [ -n "$AZURE_OPENAI_KEY" ]; then
    OPENAI_CREATE_SECRET_ARGS=(--secrets "azure-openai-api-key=$AZURE_OPENAI_KEY")
    OPENAI_KEY_ENV_PAIR="AZURE_OPENAI_API_KEY=secretref:azure-openai-api-key"
  else
    OPENAI_KEY_ENV_PAIR="AZURE_OPENAI_API_KEY="
  fi
